...
EXPECTED_STAGE_GATE="$(sed -n 's/^  stage_gate: //p' project.yaml | head -n1 | tr -d '\r')"

grep -q '"project":"Bruno Advisory"' "$HEALTH_JSON"
grep -q "\"tranche\":\"$EXPECTED_TRANCHE\"" "$HEALTH_JSON"
grep -q "\"status\":\"$EXPECTED_STATUS\"" "$HEALTH_JSON"
grep -q 'Bruno Advisory' "$CONTROL_ROOM_HTML"
grep -q "$EXPECTED_STAGE_GATE" "$CONTROL_ROOM_HTML"
...
log "Project state verification passed"
