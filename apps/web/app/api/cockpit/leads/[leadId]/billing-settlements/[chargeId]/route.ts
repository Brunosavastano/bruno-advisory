...
export async function POST(request: Request, context: { params: Promise<{ leadId: string; chargeId: string }> }) {
  ...
  if (returnTo) {
    ...
    if (!result.ok) {
      url.searchParams.set('targetedSettlementError', result.error);
      return Response.redirect(url, 303);
    }

    url.searchParams.set('targetedSettlementSuccess', result.settlement.settlementId);
    url.searchParams.set('targetedSettlementChargeId', result.charge.chargeId);
    return Response.redirect(url, 303);
  }

  if (!result.ok) {
    const status =
      result.code === 'LEAD_NOT_FOUND' || result.code === 'CHARGE_NOT_FOUND'
        ? 404
